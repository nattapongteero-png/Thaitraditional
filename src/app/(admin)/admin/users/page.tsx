...
const roleConfig: Record<Role, { label: string; color: string; icon: typeof User }> = {
  PATIENT: { label: "ผู้ป่วย", color: "bg-pale-mint text-forest-leaf", icon: User },
  DOCTOR: { label: "แพทย์", color: "bg-blue-100 text-blue-700", icon: Stethoscope },
  ADMIN: { label: "ผู้ดูแลระบบ", color: "bg-pale-mint text-forest-leaf", icon: ShieldCheck },
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 bg-forest-leaf text-white rounded-xl text-sm font-medium hover:bg-forest-leaf-hover transition-colors shadow-sm"
        >
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-50 bg-warm-sand/50">
                <th className="text-left px-5 py-3 text-xs text-muted-moss font-medium">ผู้ใช้</th>
                <th className="text-left px-5 py-3 text-xs text-muted-moss font-medium hidden md:table-cell">อีเมล</th>
                <th className="text-left px-5 py-3 text-xs text-muted-moss font-medium">บทบาท</th>
                <th className="text-left px-5 py-3 text-xs text-muted-moss font-medium hidden lg:table-cell">เข้าสู่ระบบล่าสุด</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map((user) => {
                ...
                return (
                  <tr key={user.id} className="hover:bg-pale-mint/50 transition-colors">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 bg-pale-mint rounded-full flex items-center justify-center flex-shrink-0">
                    ...
                    <td className="px-5 py-4 hidden md:table-cell">
                      <span className="text-sm text-olive-charcoal">{user.email}</span>
                    </td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4 hidden lg:table-cell">
                      <span className="text-xs text-muted-moss">{user.lastLogin}</span>
                    </td>
                    <td className="px-5 py-4">
                      <button
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center justify-end gap-1">
                        <button className="p-1.5 hover:bg-blue-50 hover:text-blue-600 rounded-lg transition-colors text-muted-moss">
                          <Edit2 className="w-4 h-4" />
                        </button>
                        <button className="p-1.5 hover:bg-pale-mint hover:text-forest-leaf rounded-lg transition-colors text-muted-moss">
                          <Shield className="w-4 h-4" />
                        </button>
                        <button className="p-1.5 hover:bg-red-50 hover:text-red-600 rounded-lg transition-colors text-muted-moss">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="p-5 space-y-4">
              <div>
                <label className="text-sm text-olive-charcoal block mb-1.5">ชื่อ-นามสกุล</label>
                <input
                  type="text"
                  value={newUser.name}
              ...
              <div>
                <label className="text-sm text-olive-charcoal block mb-1.5">อีเมล</label>
                <input
                  type="email"
                  value={newUser.email}
              ...
              <div>
                <label className="text-sm text-olive-charcoal block mb-1.5">บทบาท (Role)</label>
                <div className="relative">
                  <select
                    value={newUser.role}
                    onChange={(e) => setNewUser({ ...newUser, role: e.target.value as Role })}
                    className="w-full px-4 py-2.5 rounded-xl border border-border bg-warm-sand focus:outline-none focus:ring-2 focus:ring-forest-leaf text-sm appearance-none"
                  >
                    {(Object.keys(roleConfig) as Role[]).map((r) => (
                      <option key={r} value={r}>{roleConfig[r].label}</option>
                    ))}
                  </select>
                  <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-moss pointer-events-none" />
                </div>
              </div>
              <div className="flex gap-3 pt-2">
                <button
                  onClick={() => setShowAdd(false)}
                  className="flex-1 py-2.5 border border-border text-olive-charcoal rounded-xl text-sm hover:bg-pale-mint"
                >
                  ยกเลิก
                </button>
                <button
                  ...
                  className="flex-1 py-2.5 bg-forest-leaf text-white rounded-xl text-sm font-medium hover:bg-forest-leaf-hover flex items-center justify-center gap-2"
                >
