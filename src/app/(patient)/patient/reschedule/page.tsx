...
export default function Page() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center h-64"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-forest-leaf" /></div>}>
      <RescheduleContent />
    </Suspense>
  );
}
...
function RescheduleContent() {
  ...
  const aptId: string = searchParams.get("aptId") ?? "APT-0342";
      ...
      <div className="mx-auto">
        ...
        <div className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden mb-5">
          <div className="grid grid-cols-2 divide-x divide-gray-100">
            {/* old */}
            <div className="p-5 bg-warm-sand">
              <p className="text-xs text-muted-moss uppercase tracking-wider mb-3">นัดเดิม</p>
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-sm text-muted-moss">
        ...
        <div className="flex gap-3">
          <button
            ...
            className="flex-1 py-3 border border-border text-olive-charcoal rounded-xl font-medium hover:bg-warm-sand transition-colors"
          >
            กลับหน้านัดหมาย
          </button>
          <button
    ...
    return (
      <div className="space-y-5 pb-24 sm:pb-0">
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-sm text-muted-moss hover:text-gray-800 transition-colors"
        >
        ...
        <div className="bg-white rounded-2xl border border-border shadow-sm p-5">
          ...
          <div className="mt-4 grid grid-cols-[1fr_auto_1fr] gap-3 items-start">
            ...
            <div className="flex items-center justify-center pt-6">
              <ArrowRight className="w-5 h-5 text-muted-moss" />
            </div>
            {/* new */}
            <div className="bg-pale-mint border border-pale-mint rounded-xl p-3">
              <p className="text-xs text-forest-leaf mb-2 font-medium">นัดใหม่</p>
              <div className="space-y-1.5">
                <div className="flex items-center gap-1.5 text-xs text-forest-leaf">
                  <CalendarIcon className="w-3.5 h-3.5 flex-shrink-0" />
                  <span>{newDateLabel}</span>
                </div>
                <div className="flex items-center gap-1.5 text-xs text-forest-leaf">
          ...
          {/* reason */}
          <div className="mt-4 p-3 bg-warm-sand rounded-xl">
            <p className="text-xs text-muted-moss mb-1">เหตุผลในการเลื่อนนัด</p>
        ...
        {/* actions */}
        <div className="flex gap-3 fixed bottom-0 left-0 right-0 z-40 p-4 bg-white/95 backdrop-blur-sm border-t border-border sm:static sm:bg-transparent sm:border-0 sm:p-0 sm:backdrop-blur-none">
          <button
            onClick={() => setStep("pick")}
            className="flex-1 py-3 border border-border text-olive-charcoal rounded-xl font-medium hover:bg-warm-sand transition-colors bg-white"
          >
            แก้ไข
          </button>
          <button
            onClick={() => setStep("done")}
            className="flex-1 py-3.5 bg-forest-leaf text-white rounded-xl font-semibold hover:bg-forest-leaf-hover transition-colors shadow-sm"
          >
            ยืนยันการเลื่อนนัด
          </button>
        </div>
  ...
  return (
    <div className="space-y-5">
        ...
        {(["เลือกวันเวลา", "ยืนยัน", "เสร็จสิ้น"] as const).map((label, i) => (
          <div key={label} className="flex items-center gap-2 flex-1">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs flex-shrink-0 border-2 transition-all ${
              i === 0 ? "bg-forest-leaf border-forest-leaf text-white" : "bg-white border-gray-300 text-muted-moss"
            }`}>
              {i + 1}
            </div>
            <span className={`text-xs ${i === 0 ? "text-forest-leaf font-medium" : "text-muted-moss"}`}>{label}</span>
      ...
      <div className="bg-white rounded-2xl border border-border shadow-sm p-4">
        <p className="text-xs text-muted-moss uppercase tracking-wider mb-3">นัดหมายปัจจุบัน (#{original.id})</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        ...
        <div className="bg-white rounded-2xl border border-border shadow-sm p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-1">
              <button
                ...
                className="p-1.5 hover:bg-pale-mint rounded-lg transition-colors disabled:opacity-30"
              >
          ...
          <div className="grid grid-cols-7 gap-1">
            {daysOfWeek.map((d) => (
              <div key={d} className="text-center text-xs text-muted-moss py-2 font-medium">{d}</div>
            ))}
              ...
              return (
                <button
                  ...
                  className={`relative text-center text-sm py-2 rounded-xl transition-all
                    ...
                    ${!isSelected && !isOriginal && !isPast && day ? "hover:bg-pale-mint" : ""}
                  `}
                >
                  {day}
          ...
          <div className="flex items-center gap-4 mt-3 pt-3 border-t border-gray-50 text-xs">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-muted-moss">นัดเดิม</span>
            </div>
          </div>
        </div>
                  ...
                  return (
                    <button
                      ...
                      className={`py-2.5 rounded-xl text-sm transition-all relative
                        ${isUnavailable || isOriginalTime ? "bg-muted text-muted-moss cursor-not-allowed line-through" : ""}
                        ${isSelected ? "bg-forest-leaf text-white font-semibold" : ""}
                        ${!isUnavailable && !isSelected && !isOriginalTime ? "border border-border hover:border-emerald-400 hover:bg-pale-mint text-olive-charcoal" : ""}
                      `}
                    >
                      {slot}
                    </button>
      ...
      <div className="bg-white rounded-2xl border border-border shadow-sm p-5">
          ...
          {rescheduleReasons.map((r, i) => (
            <button
              ...
              className={`flex items-center gap-2.5 p-3 rounded-xl border-2 text-left text-sm transition-all ${
                reasonIdx === i
                  ? "border-forest-leaf bg-pale-mint text-forest-leaf-hover"
                  : "border-border text-olive-charcoal hover:border-gray-300"
              }`}
            >
              <div className={`w-4 h-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center ${
                reasonIdx === i ? "border-forest-leaf bg-forest-leaf" : "border-gray-300"
              }`}>
        ...
        {reasonIdx === rescheduleReasons.length - 1 && (
          <textarea
            ...
            className="mt-3 w-full px-4 py-3 rounded-xl border border-border bg-warm-sand focus:outline-none focus:ring-2 focus:ring-emerald-400 text-sm resize-none"
          />
        )}
      </div>
      ...
      <div className={`sticky bottom-4 bg-white rounded-2xl border shadow-lg p-4 flex flex-col sm:flex-row items-start sm:items-center gap-4 transition-all ${
        canProceed ? "border-emerald-300 shadow-pale-mint" : "border-border"
      }`}>
        <div className="flex flex-wrap gap-2 flex-1 text-sm">
          {selectedDay ? (
            <div className="flex items-center gap-1.5 bg-pale-mint text-forest-leaf px-3 py-1.5 rounded-lg">
              <CalendarIcon className="w-3.5 h-3.5" />
              {selectedDay} {months[currentMonth]} {currentYear + 543}
            </div>
          ) : (
            <div className="flex items-center gap-1.5 bg-muted text-muted-moss px-3 py-1.5 rounded-lg">
        ...
        <button
          ...
          className={`px-6 py-2.5 rounded-xl font-semibold text-sm transition-all flex-shrink-0 flex items-center gap-2 ${
            canProceed
              ? "bg-forest-leaf text-white hover:bg-forest-leaf-hover shadow-sm"
              : "bg-gray-200 text-muted-moss cursor-not-allowed"
          }`}
        >
