  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-gray-900">ตั้งค่าคลินิก</h2>
          <p className="text-gray-500 text-sm mt-1">กำหนดค่าระบบและการบูรณาการ</p>
        </div>
        <button
          onClick={handleSave}
      ...
      {activeTab === "general" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
            <h4 className="text-gray-900">ข้อมูลคลินิก</h4>
            {[
          ...
          <div className="space-y-5">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
              <h4 className="text-gray-900">Feature Flags</h4>
              {[
                ...
                <div key={f.label} className="flex items-center justify-between">
                  ...
                  <button
                    className={`w-12 h-6 rounded-full relative transition-all ${f.enabled ? "bg-emerald-500" : "bg-gray-200"}`}
                  >
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-3">
              <h4 className="text-gray-900">Session & Security</h4>
              {[
                ...
                <div key={f.label} className="flex items-center gap-3">
                  ...
                  <input
                    type="number"
                    defaultValue={f.value}
                    className="w-20 px-3 py-2 rounded-xl border border-gray-200 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-violet-400 text-sm text-center"
                  />
                </div>
              ))}
            </div>
      ...
      {activeTab === "slots" && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <div className="flex items-center justify-between mb-5">
            <h4 className="text-gray-900">ช่วงเวลาให้บริการ</h4>
            <div className="flex items-center gap-2">
              <label className="text-sm text-gray-600">ระยะเวลา Slot</label>
              <select className="px-3 py-2 rounded-xl border border-gray-200 bg-gray-50 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400">
          ...
          <div className="space-y-3">
            {slots.map((slot) => (
              <div key={slot.id} className={`flex items-center gap-4 p-4 rounded-xl border-2 transition-all ${slot.active ? "border-emerald-200 bg-emerald-50/50" : "border-gray-100 bg-gray-50/50 opacity-60"}`}>
                <div className="w-20 text-sm font-medium text-gray-700">{slot.day}</div>
                <input
                  type="time"
                  value={slot.start}
                  disabled={!slot.active}
                  onChange={(e) => setSlots((s) => s.map((sl) => sl.id === slot.id ? { ...sl, start: e.target.value } : sl))}
                  className="px-3 py-2 rounded-xl border border-gray-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-violet-400 disabled:opacity-50"
                />
                ...
                <button
                  ...
                  className={`w-11 h-6 rounded-full relative transition-all ${slot.active ? "bg-emerald-500" : "bg-gray-300"}`}
                >
      ...
      {activeTab === "integration" && (
        <div className="space-y-5">
          {[
            ...
            return (
              <div key={intg.key} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <h4 className="text-gray-900">{intg.name}</h4>
                      <button
                        onClick={() => setIntegrations((i) => ({ ...i, [intg.key]: !enabled }))}
                        className={`w-12 h-6 rounded-full relative transition-all ${enabled ? "bg-emerald-500" : "bg-gray-200"}`}
                      >
                        <div className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-all ${enabled ? "right-1" : "left-1"}`} />
                      </button>
                    </div>
                    <p className="text-sm text-gray-500 mt-0.5">{intg.desc}</p>
                    {enabled && (
                      <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                          ...
                          <input
                            ...
                            className="w-full px-3 py-2 rounded-xl border border-gray-200 bg-gray-50 text-xs focus:outline-none focus:ring-2 focus:ring-violet-400"
                          />
                        </div>
                        <div>
      ...
      {activeTab === "templates" && (
        <div className="space-y-5">
          <div className="flex justify-between items-center">
            <p className="text-sm text-gray-500">{templateList.length} templates</p>
            ...
            {templateList.map((t) => (
              <div key={t.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex items-center gap-4">
                <div className="w-9 h-9 bg-violet-100 rounded-xl flex items-center justify-center flex-shrink-0">
                  <PaperIcon className="w-4 h-4 text-violet-600" />
                </div>
                <div className="flex-1">
                  <p className="text-sm font-medium text-gray-900">{t.name}</p>
                  <div className="flex items-center gap-3 mt-0.5">
                    <span className="text-xs text-gray-400">ใช้ล่าสุด: {t.lastUsed}</span>
                    <span className="text-xs text-gray-400">ใช้งาน {t.uses} ครั้ง</span>
                  </div>
                </div>
                <div className="flex gap-2">
                  ...
                  <button className="px-3 py-1.5 text-xs bg-gray-100 text-gray-600 rounded-lg hover:bg-red-50 hover:text-red-600 transition-colors">ลบ</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {activeTab === "notifications" && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
          <h4 className="text-gray-900">การตั้งค่าการแจ้งเตือน</h4>
          {[
            ...
            <div key={n.label} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
              ...
              <button className={`w-11 h-6 rounded-full relative transition-all ${n.enabled ? "bg-emerald-500" : "bg-gray-200"}`}>
