...
const typeConfig: Record<LogType, { label: string; icon: typeof Activity; color: string }> = {
  auth: { label: "Authentication", icon: Shield, color: "text-blue-600 bg-blue-50" },
  record: { label: "Medical Record", icon: PaperIcon, color: "text-emerald-600 bg-emerald-50" },
  payment: { label: "Payment", icon: CreditCard, color: "text-amber-600 bg-amber-50" },
  signature: { label: "Digital Sign", icon: PaperIcon, color: "text-violet-600 bg-violet-50" },
  ...
  system: { label: "System", icon: Activity, color: "text-gray-600 bg-gray-100" },
};

const statusConfig = {
  success: { icon: CheckCircle2, color: "text-emerald-500 bg-emerald-50", label: "สำเร็จ" },
    ...
    <>
      <tr className={`hover:bg-gray-50/50 transition-colors ${isExpanded ? "bg-gray-50/50" : ""}`}>
        ...
        <td className="px-5 py-3">
          ...
          <p className="text-xs text-gray-400 lg:hidden">{log.user}</p>
        </td>
        <td className="px-5 py-3 hidden md:table-cell">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-gray-600 truncate max-w-32">{log.user}</span>
          </div>
        </td>
        ...
        <tr className="bg-gray-50/80">
          <td colSpan={7} className="px-5 py-3">
            <div className="flex items-start gap-3">
              <div className="text-xs font-mono text-gray-500 bg-white rounded-lg px-3 py-2 border border-gray-100 flex-1">
                <span className="text-violet-600">LOG_ID:</span> {log.id}&nbsp;&nbsp;
                <span className="text-violet-600">Details:</span> {log.details}
              </div>
            </div>
          </td>
        </tr>
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">บันทึกการใช้งานระบบและการเข้าถึงเวชระเบียน</p>
        </div>
        <button className="flex items-center gap-2 px-4 py-2.5 bg-white border border-gray-200 text-gray-700 rounded-xl text-sm font-medium hover:bg-gray-50 transition-colors shadow-sm">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "รวมทั้งหมด", value: logs.length, color: "text-gray-900" },
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {(["all", "success", "warning", "error"] as const).map((s) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs transition-all ${
                statusFilter === s
                  ? s === "all" ? "bg-slate-700 text-white" : s === "success" ? "bg-emerald-600 text-white" : s === "warning" ? "bg-amber-500 text-white" : "bg-red-600 text-white"
                  : "bg-white border border-gray-200 text-gray-600 hover:bg-gray-50"
              }`}
            >
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-50 bg-gray-50/50">
                <th className="text-left px-5 py-3 text-xs text-gray-500 font-medium">เวลา</th>
                <th className="text-left px-5 py-3 text-xs text-gray-500 font-medium">ประเภท</th>
                <th className="text-left px-5 py-3 text-xs text-gray-500 font-medium">Action</th>
                <th className="text-left px-5 py-3 text-xs text-gray-500 font-medium hidden md:table-cell">ผู้ใช้</th>
                <th className="text-left px-5 py-3 text-xs text-gray-500 font-medium hidden lg:table-cell">IP</th>
