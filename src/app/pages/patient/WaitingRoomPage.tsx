...
export function WaitingRoomPage() {
        ...
        <div className="flex-1 relative bg-slate-800 flex items-center justify-center">
          ...
          <div className="absolute bottom-4 right-4 w-36 h-24 bg-gray-700 rounded-xl border-2 border-white/20 flex items-center justify-center">
            {camOn ? (
              ...
              <div className="text-center">
                <CameraOffIcon className="w-6 h-6 text-gray-400 mx-auto" />
                <p className="text-gray-400 text-xs mt-1">กล้องปิด</p>
              </div>
            )}
          </div>
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-gray-900">ห้องรอพบแพทย์</h2>
        <p className="text-gray-500 text-sm mt-1">กรุณารอจนกว่าแพทย์จะรับการเชื่อมต่อ</p>
      </div>
            ...
            <div key={s.id} className="flex flex-col items-center gap-2 z-10 flex-1">
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all
                  ${s.done ? "bg-emerald-600 border-emerald-600" : ""}
                  ${s.active ? "bg-white border-emerald-600 shadow-md shadow-emerald-100" : ""}
                  ${!s.done && !s.active ? "bg-white border-gray-300" : ""}`}
              >
                {s.done ? (
                  <CheckCircle2 className="w-5 h-5 text-white" />
                ) : (
                  <div className={`w-3 h-3 rounded-full ${s.active ? "bg-emerald-600 animate-pulse" : "bg-gray-300"}`} />
                )}
              </div>
              <p className={`text-xs font-medium ${s.done || s.active ? "text-emerald-700" : "text-gray-400"}`}>
                {s.label}
              </p>
            </div>
          ))}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="bg-gradient-to-r from-emerald-700 to-teal-600 p-6 text-center text-white">
          <p className="text-emerald-100 text-sm">ลำดับคิวของคุณ</p>
          <p className="text-6xl font-bold mt-1">3</p>
          <p className="text-emerald-100 text-sm mt-2">ยังมีผู้รอก่อนคุณ 2 คน</p>
        </div>
        <div className="grid grid-cols-3 divide-x divide-gray-100">
          {[
            ...
            { icon: CheckCircle2, label: "เสร็จแล้ว", value: "5 คิว", color: "text-emerald-600" },
          ].map((item) => {
      ...
      {/* Device Check */}
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <h4 className="text-gray-900 mb-4">ตรวจสอบอุปกรณ์</h4>
        <div className="space-y-3">
          {[
            { label: "กล้อง (Camera)", status: camOn, toggle: () => setCamOn(!camOn) },
            { label: "ไมโครโฟน (Microphone)", status: micOn, toggle: () => setMicOn(!micOn) },
          ].map((device) => (
            <div key={device.label} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
              <div className="flex items-center gap-3">
                <div
                  className={`w-2.5 h-2.5 rounded-full ${device.status ? "bg-green-500" : "bg-red-500"}`}
                />
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none ${device.status ? "bg-emerald-500" : "bg-gray-300"}`}
                >
                  <span
          ...
          <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm text-gray-700">การเชื่อมต่ออินเทอร์เน็ต</span>
            </div>
            <div className="flex items-center gap-2">
      ...
      <div className="sticky bottom-4 space-y-3 bg-gray-50/80 backdrop-blur-sm rounded-2xl pt-3">
        {/* Warning */}
        ...
        <button
          ...
          className="w-full py-4 bg-emerald-700 hover:bg-emerald-800 text-white rounded-2xl font-semibold flex items-center justify-center gap-3 transition-colors shadow-lg text-base"
        >
