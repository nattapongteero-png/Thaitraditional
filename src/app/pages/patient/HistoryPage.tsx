  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-gray-900">ประวัติการรักษา</h2>
          <p className="text-gray-500 text-sm mt-1">ดูประวัติการพบแพทย์และสถานะยา</p>
        </div>
        <div className="relative">
          ...
          <input
            ...
            className="pl-10 pr-4 py-2.5 rounded-xl border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-emerald-400 text-sm w-full sm:w-60"
          />
        </div>
      </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-xs bg-emerald-100 text-emerald-700 px-2.5 py-1 rounded-lg font-medium">
                      {visit.id}
                    </span>
                  ...
                  <p className="font-semibold text-gray-900 mt-2">{visit.diagnosis}</p>
                ...
                <div className="flex sm:flex-col gap-2">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-2 bg-emerald-50 text-emerald-700 rounded-xl text-sm hover:bg-emerald-100 transition-colors"
                  >
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between p-5 border-b border-gray-100">
              <div>
                <h3 className="text-gray-900">ติดตามการจัดส่งยา</h3>
                <p className="text-xs text-gray-500 mt-0.5">{selectedVisit.id}</p>
              </div>
              <button
            ...
            <div className="p-5">
              ...
              <div className="mb-5 bg-amber-50 rounded-xl p-4">
                ...
                {selectedVisit.medicines.map((m) => (
                  <p key={m} className="text-sm text-gray-700 flex items-center gap-2">
              ...
              <div className="bg-gray-50 rounded-xl p-3 mb-5 flex items-center justify-between">
                <span className="text-xs text-gray-500">หมายเลขพัสดุ</span>
                <span className="text-sm font-mono font-semibold text-gray-900">{selectedVisit.trackingNo}</span>
              </div>
                    ...
                    <div key={step.key} className="flex items-center gap-3">
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0
                          ${isDone ? (isCurrent ? "bg-emerald-600 shadow-md" : "bg-emerald-200") : "bg-gray-200"}`}
                      >
                        <Icon className={`w-4 h-4 ${isDone ? (isCurrent ? "text-white" : "text-emerald-700") : "text-gray-400"}`} />
                      </div>
                      <div className="flex-1">
                        <p className={`text-sm ${isDone ? "text-gray-900 font-medium" : "text-gray-400"}`}>
                          {step.label}
                          {isCurrent && (
                            <span className="ml-2 text-xs text-emerald-600 font-normal">• ปัจจุบัน</span>
                          )}
                        </p>
                      </div>
