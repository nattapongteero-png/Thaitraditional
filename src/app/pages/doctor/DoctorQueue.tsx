...
import patientFemale from "figma:asset/df8bf5999c1401542d3a02a783a7254d76a0a4dd.png";
import patientMale from "figma:asset/411c3b8f4e17ed8d3913383365e850ae5990c8f9.png";
...
const statusConfig: Record<Status, { label: string; color: string; icon: typeof CheckCircle2 }> = {
  ...
  done: { label: "เสร็จแล้ว", color: "bg-emerald-100 text-emerald-700", icon: CheckCircle2 },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-gray-900">คิววันนี้</h2>
          <p className="text-gray-500 text-sm mt-1">จันทร์ที่ 2 มีนาคม 2569</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: "ทั้งหมด", value: stats.total, color: "text-gray-900", bg: "bg-white" },
          { label: "รอพบแพทย์", value: stats.waiting, color: "text-amber-700", bg: "bg-amber-50", border: "border-amber-200" },
          { label: "เสร็จแล้ว", value: stats.done, color: "text-emerald-700", bg: "bg-emerald-50", border: "border-emerald-200" },
          { label: "ไม่มา", value: stats.noShow, color: "text-red-700", bg: "bg-red-50", border: "border-red-200" },
        ].map((s) => (
          <div key={s.label} className={`${s.bg} rounded-xl p-4 border ${s.border || "border-gray-100"} shadow-sm`}>
      ...
      {stats.inProgress > 0 && (
        <div className="bg-blue-600 text-white rounded-2xl p-4 flex items-center gap-4">
          ...
          <button
            onClick={() => navigate("/doctor/visit/P003")}
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="pl-10 pr-4 py-2.5 rounded-xl border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-blue-400 text-sm w-full"
          />
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-50 bg-gray-50/50">
                <th className="text-left px-5 py-3 text-xs text-gray-500 font-medium">คิว</th>
                <th className="text-left px-5 py-3 text-xs text-gray-500 font-medium">ผู้ป่วย</th>
                <th className="text-left px-5 py-3 text-xs text-gray-500 font-medium hidden md:table-cell">HN</th>
                <th className="text-left px-5 py-3 text-xs text-gray-500 font-medium hidden lg:table-cell">ประเภท</th>
              ...
              {filtered.map((patient) => {
                ...
                return (
                  <tr key={patient.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">{patient.age} ปี</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-4 hidden md:table-cell">
                      <span className="text-xs font-mono text-gray-600">{patient.hn}</span>
                    </td>
                    ...
                    <td className="px-5 py-4">
                      <div>
                        <p className="text-sm text-gray-900">{patient.time} น.</p>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center justify-end gap-2">
                        {patient.status === "waiting" && (
                          <>
                            <button
                              onClick={() => navigate(`/doctor/visit/${patient.id}`)}
